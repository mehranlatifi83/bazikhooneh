...
public final class EmailVerificationActivity extends NavigableActivity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        client = new AccountManagementClient(BuildConfig.API_BASE_URL, new SessionStore(this).token(), listener);
    ...
    private void requestCode() { try { client.post("request", "/api/v1/accounts/email/request/", new JSONObject().put("email", text(R.id.security_email))); } catch (JSONException ignored) {} }
    private void confirm() { try { client.post("confirm", "/api/v1/accounts/email/confirm/", new JSONObject().put("code", text(R.id.security_code))); } catch (JSONException ignored) {} }
    private void showRequestStep() { findViewById(R.id.email_request_step).setVisibility(View.VISIBLE); findViewById(R.id.email_confirm_step).setVisibility(View.GONE); status.setText(""); }
    private void showConfirmStep(JSONObject json) {
        ...
        ((TextView) findViewById(R.id.email_code_destination)).setText(getString(R.string.code_sent_to, text(R.id.security_email)));
    ...
    private final AccountManagementClient.Listener listener = new AccountManagementClient.Listener() {
        public void onSuccess(String operation, JSONObject json) { runOnUiThread(() -> { if ("request".equals(operation)) { showConfirmStep(json); status.setText(R.string.verification_sent); } else status.setText(R.string.changes_saved); }); }
        public void onError(String error) { runOnUiThread(() -> status.setText(AccountErrorMessages.get(EmailVerificationActivity.this, error))); }
    };
}
