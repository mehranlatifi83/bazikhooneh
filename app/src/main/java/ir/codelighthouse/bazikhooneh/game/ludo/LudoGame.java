...
public final class LudoGame {
    ...
    public List<String> captureTargets(int piece){List<String> result=new ArrayList<>();if(!legalPieces().contains(piece))return result;int before=progress[currentPlayer][piece],after=before==HOME?0:before+die,global=globalPosition(currentPlayer,after);if(global<0||isSafe(global))return result;for(int other=0;other<PLAYERS;other++)if(other!=currentPlayer)for(int target=0;target<PIECES;target++)if(globalPosition(other,progress[other][target])==global)result.add(other+":"+target);return result;}
    public int captureCount(int piece){return captureTargets(piece).size();}
