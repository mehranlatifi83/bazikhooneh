...
public final class LudoGame {
    ...
    public int captureCount(int piece){if(!legalPieces().contains(piece))return 0;int before=progress[currentPlayer][piece],after=before==HOME?0:before+die,global=globalPosition(currentPlayer,after),count=0;if(global<0||isSafe(global))return 0;for(int other=0;other<PLAYERS;other++)if(other!=currentPlayer)for(int target=0;target<PIECES;target++)if(globalPosition(other,progress[other][target])==global)count++;return count;}
