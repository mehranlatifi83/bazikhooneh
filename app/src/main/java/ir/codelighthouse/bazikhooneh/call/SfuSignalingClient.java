...
final class SfuSignalingClient {
  ...
  SfuSignalingClient(
                  ...
                  if (future != null) {
                    ...
                    if (error.isEmpty()) {
                      ...
                      if (data instanceof JSONObject) future.complete((JSONObject) data);
                        ...
                        try {
                          wrapper.put("values", data);
                        } catch (Exception ignored) {
                        }
                        future.complete(wrapper);
                      } else future.complete(new JSONObject());
                    } else future.completeExceptionally(new IllegalStateException(error));
                  }
                  return;
                }
    ...
    String ws =
        baseUrl.replaceFirst("^https", "wss").replaceFirst("^http", "ws").replaceAll("/$", "");
    Request request =
        new Request.Builder()
  ...
  JSONObject request(String action, JSONObject data) throws Exception {
    ...
    if (!socket.send(message)) {
      pending.remove(id);
      throw new IllegalStateException("sfu_disconnected");
    }
    JSONObject result = future.get(15, TimeUnit.SECONDS);
  ...
  private void failPending() {
    for (CompletableFuture<JSONObject> value : pending.values())
      value.completeExceptionally(new IllegalStateException("sfu_disconnected"));
    pending.clear();
  }
}
