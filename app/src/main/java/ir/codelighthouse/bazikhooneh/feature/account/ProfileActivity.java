...
import ir.codelighthouse.bazikhooneh.feature.social.SafetyActivity;
...
public final class ProfileActivity extends NavigableActivity {
  private static final String[] COLORS = {
    "#2E7D32", "#C62828", "#1565C0", "#6A1B9A", "#EF6C00", "#455A64"
  };
  ...
  protected void onCreate(Bundle state) {
    ...
    ArrayAdapter<CharSequence> adapter =
        ArrayAdapter.createFromResource(
            this, R.array.avatar_colors, android.R.layout.simple_spinner_item);
    ...
    findViewById(R.id.profile_security)
        .setOnClickListener(v -> startActivity(new Intent(this, AccountSecurityActivity.class)));
    findViewById(R.id.profile_safety)
        .setOnClickListener(v -> startActivity(new Intent(this, SafetyActivity.class)));
    findViewById(R.id.profile_retry)
        .setOnClickListener(
            v -> {
              findViewById(R.id.profile_loading).setVisibility(View.VISIBLE);
              findViewById(R.id.profile_retry).setVisibility(View.GONE);
              findViewById(R.id.profile_error).setVisibility(View.GONE);
              client.load();
            });
    client.load();
  }
  ...
  private final ProfileClient.Listener listener =
      new ProfileClient.Listener() {
        @Override
        public void onProfile(JSONObject profile) {
          runOnUiThread(() -> renderProfile(profile));
        }

        @Override
        public void onHistory(JSONObject value) {
        ...
        public void onError() {
          runOnUiThread(
              () -> {
                findViewById(R.id.profile_loading).setVisibility(View.GONE);
                TextView error = findViewById(R.id.profile_error);
                error.setText(R.string.profile_load_error);
                error.setVisibility(View.VISIBLE);
                findViewById(R.id.profile_retry).setVisibility(View.VISIBLE);
                error.announceForAccessibility(getString(R.string.profile_load_error));
              });
        }
      };
  ...
  private void renderProfile(JSONObject profile) {
    try {
      ...
      stats.setText(
          getString(
              R.string.profile_stats_value,
              values.getInt("played"),
              values.getInt("wins"),
              values.getInt("losses")));
      int colorIndex = 0;
  ...
  private void renderHistory(JSONObject response) {
    try {
      ...
      for (int i = 0; i < results.length(); i++) {
        ...
        String result =
            "win".equals(match.getString("result"))
                ? getString(R.string.match_win)
                : getString(R.string.match_loss);
        if (i > 0) text.append("\n");
        if ("ludo".equals(match.optString("game_key"))) {
          ...
          text.append(
              getString(
                  R.string.match_history_item,
                  result,
                  opponent.getString("display_name"),
                  match.getInt("round")));
        }
      }
  ...
  private void logout(String token) {
    new AccountClient(
            ...
            new AccountClient.Listener() {
              @Override
              public void onSession(AccountSession session) {}
