...
public class RoomActivity extends NavigableActivity implements CommunityClient.Events {
  ...
  protected void onCreate(Bundle state) {
    ...
    findViewById(R.id.community_start_tic)
        .setOnClickListener(v -> startGame("three_piece_tic_tac_toe"));
    ...
    findViewById(R.id.community_history)
        .setOnClickListener(
            v ->
                startActivity(
  ...
  private void renderRoom(JSONObject room) {
    ...
    findViewById(R.id.community_start_call)
        .setVisibility(moderator && !callActive ? View.VISIBLE : View.GONE);
    ...
    JSONObject activeGame = room.optJSONObject("active_game");
    View panel = findViewById(R.id.community_active_game_panel);
    panel.setVisibility(activeGame == null ? View.GONE : View.VISIBLE);
    if (activeGame != null) {
      String gameName =
          getString(
              "ludo".equals(activeGame.optString("game_key"))
                  ? R.string.ludo_title
                  : R.string.tic_tac_toe_title);
      JSONArray participants = activeGame.optJSONArray("participants");
      ((TextView) findViewById(R.id.community_active_game_status))
          .setText(
              getString(
                  R.string.active_room_game,
                  gameName,
                  participants == null ? 0 : participants.length(),
                  activeGame.optInt("capacity")));
      ((Button) findViewById(R.id.community_join_game))
          .setText(
              activeGame.optBoolean("is_participant")
                  ? R.string.resume_active_game
                  : R.string.join_active_game);
    }
    ...
    if (values != null)
      for (int i = 0; i < values.length(); i++) {
        ...
        row.setText(
            getString(
                R.string.community_member_item,
        ...
        members.addView(row);
        if (moderator
            && !item.optString("username").equalsIgnoreCase(new SessionStore(this).username())) {
          row.setOnLongClickListener(
              v -> {
                showMemberMenu(row, item.optString("username"), item.optString("role"));
                return true;
              });
        }
      }
    JSONArray requests = room.optJSONArray("pending_join_requests");
    if (moderator && requests != null && requests.length() > 0)
      showJoinRequest(requests.optJSONObject(0));
  }

  private void showJoinRequest(JSONObject request) {
    if (request == null || shownJoinRequest == request.optLong("id")) return;
    shownJoinRequest = request.optLong("id");
    JSONObject account = request.optJSONObject("account");
    String name = account == null ? "" : account.optString("display_name");
    new android.app.AlertDialog.Builder(this)
        .setMessage(getString(R.string.join_request_message, name))
        .setNegativeButton(R.string.reject, (d, w) -> resolveRequest(request.optLong("id"), false))
        .setPositiveButton(R.string.approve, (d, w) -> resolveRequest(request.optLong("id"), true))
        .show();
  }

  private void resolveRequest(long id, boolean approve) {
    client.resolveJoinRequest(
        code,
        id,
        approve,
        (data, error) ->
            runOnUiThread(
                () -> {
                  ...
                  else client.details(code, (room, e) -> runOnUiThread(() -> renderRoom(room)));
                }));
  }
  ...
  private void showRoomSettings() {
    ...
    title.setHint(R.string.community_room_name);
    title.setText(currentRoom.optString("title"));
    form.addView(title);
    CheckBox publicRoom = new CheckBox(this);
    publicRoom.setText(R.string.public_room);
    publicRoom.setChecked("public".equals(currentRoom.optString("privacy")));
    form.addView(publicRoom);
    CheckBox approval = new CheckBox(this);
    approval.setText(R.string.require_join_approval);
    approval.setChecked("request".equals(currentRoom.optString("join_policy")));
    form.addView(approval);
    new android.app.AlertDialog.Builder(this)
        ...
        .setPositiveButton(
            ...
            (d, w) ->
                client.updateRoom(
                    ...
                    publicRoom.isChecked() ? "public" : "private",
                    approval.isChecked() ? "request" : "open",
                    (room, error) ->
                        runOnUiThread(
                            () -> {
                              if (error != null) show(error);
  ...
  private void confirmLeave() {
    new android.app.AlertDialog.Builder(this)
        .setMessage(R.string.leave_room_confirmation)
        .setNegativeButton(android.R.string.cancel, null)
        .setPositiveButton(
            R.string.leave_room,
            (d, w) ->
  ...
  private void showMemberMenu(View anchor, String username, String role) {
    PopupMenu menu = new PopupMenu(this, anchor);
    menu.getMenu().add(R.string.mute_room_chat);
    menu.getMenu().add(R.string.ban_from_call);
    menu.getMenu().add(R.string.ban_from_room);
    if ("member".equals(role)) menu.getMenu().add(R.string.make_room_admin);
    else if ("admin".equals(role)) menu.getMenu().add(R.string.remove_room_admin);
    menu.setOnMenuItemClickListener(
        item -> {
          int id = item.getTitle().toString().hashCode();
          String action;
          ...
          else if (item.getTitle().equals(getString(R.string.make_room_admin))) action = "promote";
          else action = "demote";
          client.moderateRoom(
              code,
              username,
  ...
  private void showMessageMenu(View anchor, JSONObject message) {
    PopupMenu menu = new PopupMenu(this, anchor);
    ...
    if (own) menu.getMenu().add(R.string.edit_message);
    if (own || moderator) menu.getMenu().add(R.string.delete_message);
    menu.setOnMenuItemClickListener(
        item -> {
          if (item.getTitle().equals(getString(R.string.reply_message))) selectReply(message);
          else if (item.getTitle().equals(getString(R.string.edit_message))) editMessage(message);
          else deleteMessage(message.optLong("id"));
          return true;
        });
    menu.show();
  }

  private void selectReply(JSONObject message) {
    replyToMessage = message.optLong("id");
    JSONObject sender = message.optJSONObject("sender");
    EditText input = findViewById(R.id.community_message_input);
    input.setHint(
        getString(R.string.replying_to, sender == null ? "" : sender.optString("display_name")));
    input.requestFocus();
  }

  private void editMessage(JSONObject message) {
    final EditText input = new EditText(this);
    input.setText(message.optString("text"));
  ...
  private void updateMessage(JSONObject item) {
    long id = item.optLong("id");
    ...
    row.setText(
        getString(
            R.string.community_message_item,
            sender == null ? "" : sender.optString("display_name"),
            item.optString("text")));
  }

  private void removeMessage(long id) {
    View container = messageContainers.remove(id);
    messageViews.remove(id);
    messageActionViews.remove(id);
    if (container != null) {
      messages.removeView(container);
      show(getString(R.string.message_removed));
    }
  }

  private void scrollBottom() {
    findViewById(R.id.community_chat_scroll)
        .post(
            () ->
                ((ScrollView) findViewById(R.id.community_chat_scroll))
        ...
        (data, error) ->
            runOnUiThread(
                () -> {
                  ...
                  renderMessages(data.optJSONArray("results"));
                }));
  }
  ...
  private void send() {
    ...
    Button button = findViewById(R.id.community_send);
    button.setEnabled(false);
    long reply = replyToMessage;
    client.sendMessage(
        code,
        text,
        reply,
  ...
  private void inviteFriend() {
    ...
    username.setHint(R.string.account_username);
    new android.app.AlertDialog.Builder(this)
        .setTitle(R.string.invite_friend_to_room)
        .setMessage(R.string.invite_friend_help)
        .setView(username)
        .setNegativeButton(android.R.string.cancel, null)
        .setPositiveButton(
                  ...
                  (data, error) ->
                      runOnUiThread(
                          () ->
                              show(
                                  error == null
                                      ? getString(R.string.room_invitation_sent, value)
                                      : error)));
            })
        .show();
  }
        ...
        (data, error) ->
            runOnUiThread(
                () -> {
                  ...
                  openGamePayload(data);
                }));
  }

  private void openGamePayload(JSONObject data) {
    if (data == null) return;
    String key = data.optString("game_key");
    JSONObject player = data.optJSONObject("player");
    if ("ludo".equals(key)) openLudo(player);
    else openTic(player);
  }
  ...
  private void openTic(JSONObject player) {
    ...
    startActivity(
        new Intent(this, TicTacToeGameActivity.class)
            .putExtra(TicTacToeGameActivity.EXTRA_MODE, "online"));
  }

  private void openLudo(JSONObject player) {
    if (player == null) return;
  ...
  private void joinSelectedGame(JSONObject event) {
    if (startingGame) {
      startingGame = false;
      return;
    }
    String name =
        getString(
            "ludo".equals(event.optString("game_key"))
                ? R.string.ludo_title
                : R.string.tic_tac_toe_title);
    new android.app.AlertDialog.Builder(this)
        .setMessage(getString(R.string.room_game_started, name))
        .setNegativeButton(R.string.not_now, null)
        .setPositiveButton(R.string.join_active_game, (d, w) -> joinActiveGame())
        .show();
  }
  ...
  public void onOpen() {
    runOnUiThread(
        () -> {
          ...
          if (!client.ping()) scheduleReconnect();
        });
  }
    ...
    runOnUiThread(
        () -> {
          ...
          if ("room_state".equals(type)) renderRoom(event.optJSONObject("room"));
            ...
            if (!everConnected) {
              everConnected = true;
              show(getString(R.string.room_connected));
            } else {
              status.setText(R.string.room_connected);
              refreshMessages();
              client.details(
                  code,
                  (data, error) ->
          ...
          else if ("game_participant_joined".equals(type))
            client.details(
                code,
                (d, e) ->
                    runOnUiThread(
                        () -> {
                          if (d != null) renderRoom(d);
                        }));
          else if ("join_request".equals(type) && moderator) {
            JSONObject request = new JSONObject();
            try {
              request.put("id", event.optLong("request_id"));
              request.put("account", event.optJSONObject("account"));
            } catch (Exception ignored) {
            }
            showJoinRequest(request);
          } else if ("room_updated".equals(type))
            client.details(
                code,
                (d, e) ->
                    runOnUiThread(
          ...
          else if ("call_started".equals(type) || "call_ended".equals(type))
            client.details(
                code,
                (d, e) ->
                    runOnUiThread(
              ...
              || "member_left".equals(type)
              || "moderation".equals(type))
            client.details(
                code,
                (d, e) ->
                    runOnUiThread(
            ...
            show(event.optString("error", getString(R.string.error_generic)));
        });
  }

  @Override
  public void onClosed() {
    runOnUiThread(this::scheduleReconnect);
  }
  ...
  private void scheduleReconnect() {
    ...
    status.setText(R.string.room_reconnecting);
    reconnectHandler.removeCallbacksAndMessages(null);
    reconnectHandler.postDelayed(
        () -> {
          if (reconnectScheduled && !destroyed)
            status.announceForAccessibility(getString(R.string.room_reconnecting));
        },
        8000);
  }
  ...
  protected void onResume() {
    super.onResume();
    if (client != null) refreshMessages();
  }
