...
public class RoomListActivity extends NavigableActivity {
  ...
  private void joinEnteredRoom() {
    String code =
        normalizedRoomCode(((EditText) findViewById(R.id.community_code)).getText().toString());
    if (code.isEmpty()) {
  ...
  private String selectedGameKey() {
    ...
    return position >= 0 && position < onlineGames.size() ? onlineGames.get(position).id : "";
  }
  ...
  private void setLoading(boolean loading) {
    findViewById(R.id.community_progress).setVisibility(loading ? View.VISIBLE : View.GONE);
