package ir.codelighthouse.bazikhooneh.feature.home;
...
import ir.codelighthouse.bazikhooneh.core.ui.AppDisplay;
import ir.codelighthouse.bazikhooneh.feature.account.LoginActivity;
import ir.codelighthouse.bazikhooneh.feature.account.ProfileActivity;
import ir.codelighthouse.bazikhooneh.feature.settings.LanguageActivity;
...
import ir.codelighthouse.bazikhooneh.feature.social.LeaderboardActivity;
...
import ir.codelighthouse.bazikhooneh.notification.NotificationJobService;
...
public final class HomeActivity extends Activity {
  ...
  protected void onCreate(Bundle state) {
    ...
    renderGames();
    findViewById(R.id.open_profile)
        .setOnClickListener(
            v -> {
              Class<?> destination =
                  new SessionStore(this).isSignedIn() ? ProfileActivity.class : LoginActivity.class;
              startActivity(new Intent(this, destination));
            });
    findViewById(R.id.open_settings)
        .setOnClickListener(v -> startActivity(new Intent(this, SettingsActivity.class)));
    findViewById(R.id.open_community_rooms)
        .setOnClickListener(v -> AppNavigator.openRooms(this, ""));
    findViewById(R.id.open_guide)
        .setOnClickListener(v -> startActivity(new Intent(this, GuideActivity.class)));
    findViewById(R.id.open_friends)
        .setOnClickListener(
            v -> {
              Class<?> destination =
                  new SessionStore(this).isSignedIn() ? FriendsActivity.class : LoginActivity.class;
              startActivity(new Intent(this, destination));
            });
    findViewById(R.id.open_notifications)
        .setOnClickListener(
            v -> {
              Class<?> destination =
                  new SessionStore(this).isSignedIn()
                      ? NotificationsActivity.class
                      : LoginActivity.class;
              startActivity(new Intent(this, destination));
            });
    findViewById(R.id.open_leaderboard)
        .setOnClickListener(
            v -> {
              Class<?> destination =
                  new SessionStore(this).isSignedIn()
  ...
  protected void onResume() {
    ...
    welcome.setText(
        displayName.isEmpty()
            ? getString(R.string.home_guest)
            : getString(R.string.home_welcome, displayName));
    ((Button) findViewById(R.id.open_profile))
        .setText(
  ...
  private void refreshOrUpgrade(SessionStore store, boolean upgrade) {
    ...
    if (upgrade) client.upgrade(store.token());
    else client.refresh(store.refreshToken());
  }
  ...
  private void renderGames() {
    ...
    for (GameDefinition game : GameCatalog.availableGames()) {
      ...
      card.setText(
          getString(
              R.string.game_card_content,
              getString(game.titleRes),
