...
public final class RoomEventsActivity extends NavigableActivity {
  ...
  protected void onCreate(Bundle state) {
    ...
    TextView status = findViewById(R.id.room_events_status);
    LinearLayout list = findViewById(R.id.room_events_list);
    new CommunityClient(this, BuildConfig.API_BASE_URL, store.token())
        .events(
            code,
            (data, error) ->
                runOnUiThread(
                    () -> {
                      ...
                      if (values == null || values.length() == 0) {
                        status.setText(R.string.no_room_history);
                        return;
                      }
                      ...
                      for (int i = values.length() - 1; i >= 0; i--) {
                        ...
                        JSONObject actor = value.optJSONObject("actor");
                        String name =
                            actor == null
                                ? getString(R.string.system_actor)
                                : actor.optString("display_name");
                        TextView row = new TextView(this);
                        row.setText(describe(value.optString("kind"), name));
                        row.setTextSize(17);
  ...
  private String describe(String kind, String actor) {
    switch (kind) {
      case "room_created":
        return getString(R.string.event_room_created, actor);
      case "member_joined":
        return getString(R.string.event_member_joined, actor);
      case "member_left":
        return getString(R.string.event_member_left, actor);
      case "call_started":
        return getString(R.string.event_call_started, actor);
      case "game_selected":
        return getString(R.string.event_game_selected, actor);
      case "game_action":
        return getString(R.string.event_game_action, actor);
      default:
        return getString(R.string.event_generic, actor, kind.replace('_', ' '));
    }
  }
}
