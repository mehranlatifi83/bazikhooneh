...
public final class CommunityClient {
  public interface Callback {
    void complete(JSONObject data, String error);
  }
  ...
  public void list(Callback callback) {
    request("GET", "/api/v1/community/rooms/", null, callback);
  }
  ...
  public void updateRoom(
      String code, String title, String privacy, String joinPolicy, Callback callback) {
    JSONObject body = new JSONObject();
    try {
      body.put("title", title);
      body.put("privacy", privacy);
      body.put("join_policy", joinPolicy);
    } catch (Exception ignored) {
    }
  ...
  public void resolveJoinRequest(String code, long requestId, boolean approve, Callback callback) {
    JSONObject body = new JSONObject();
    try {
      body.put("action", approve ? "approve" : "deny");
    } catch (Exception ignored) {
    }
    request(
        "POST",
        "/api/v1/community/rooms/" + code + "/join-requests/" + requestId + "/",
        body,
        callback);
  }
  ...
  public void sendMessage(String code, String text, long replyTo, Callback callback) {
    ...
    try {
      body.put("text", text);
      if (replyTo > 0) body.put("reply_to", replyTo);
    } catch (Exception ignored) {
    }
  ...
  public void stats(Callback callback) {
    request("GET", "/api/v1/stats/me/", null, callback);
  }

  public void leaderboard(Callback callback) {
    request("GET", "/api/v1/leaderboard/", null, callback);
  }

  public void startCall(String code, boolean requestToSpeak, Callback callback) {
    JSONObject body = new JSONObject();
    try {
      body.put("mic_policy", requestToSpeak ? "request" : "open");
    } catch (Exception ignored) {
    }
  ...
  public void matchmakingStatus(Callback callback) {
    request("GET", "/api/v1/matchmaking/", null, callback);
  }

  public void cancelMatchmaking(Callback callback) {
    request("DELETE", "/api/v1/matchmaking/", null, callback);
  }
  ...
  public void raiseHand() {
    sendType("call.raise_hand");
  }

  public void sendMediaState(boolean mic) {
    JSONObject value = new JSONObject();
    try {
      value.put("type", "call.media_state");
      value.put("mic_enabled", mic);
      send(value);
    } catch (Exception ignored) {
    }
  }
  ...
  public void sendSignal(String to, String type, JSONObject payload) {
    JSONObject value = new JSONObject();
    try {
      value.put("type", "call.signal");
      value.put("to", to);
      value.put("signal_type", type);
  ...
  private static final class CallbackAdapter implements okhttp3.Callback {
    ...
    public void onResponse(Call call, Response response) {
      try (ResponseBody body = response.body()) {
        ...
        callback.complete(null, "invalid_server_response");
      }
    }
  }
}
