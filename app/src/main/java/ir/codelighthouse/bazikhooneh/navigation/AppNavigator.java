...
public final class AppNavigator {
  ...
  public static void openRoom(Context context, String roomCode) {
    ...
    Intent intent =
        authenticatedDestination(context).putExtra(RoomListActivity.EXTRA_ROOM_CODE, roomCode);
    context.startActivity(intent);
  }
  ...
  public static boolean consumePendingRoom(Activity activity) {
    ...
    activity.startActivity(
        new Intent(activity, RoomListActivity.class)
  ...
  private static Intent authenticatedDestination(Context context) {
    Class<?> destination =
        new SessionStore(context).isSignedIn() ? RoomListActivity.class : LoginActivity.class;
  ...
  private static void rememberRoom(Context context, String roomCode) {
    context
        .getSharedPreferences(PREFERENCES, Context.MODE_PRIVATE)
        .edit()
        .putString(PENDING_ROOM, roomCode)
        .apply();
  }
}
