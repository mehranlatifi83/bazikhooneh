...
public final class OnlineLobbyActivity extends NavigableActivity {
  ...
  protected void onCreate(Bundle state) {
    ...
    findViewById(R.id.lobby_create)
        .setOnClickListener(
            v -> {
              setLoading(true);
              client.createRoom();
  ...
  private void join() {
    ...
    if (code.length() != 6) {
      showStatus(getString(R.string.room_code_required));
      return;
    }
    setLoading(true);
  ...
  private final OnlineGameClient.Listener listener =
      new OnlineGameClient.Listener() {
          ...
          runOnUiThread(
              () -> {
                SecurePreferences secure =
                    SecurePreferences.open(OnlineLobbyActivity.this, "online_session");
                secure.putString("room", session.game.roomCode);
                secure.putString("symbol", session.symbol);
                secure.putString("token", session.token);
                openingGame = true;
                startActivity(
                    new Intent(OnlineLobbyActivity.this, TicTacToeGameActivity.class)
        ...
        public void onDisconnected() {
          if (!openingGame) runOnUiThread(() -> setLoading(false));
        }
        ...
        public void onError(String error) {
          runOnUiThread(
              () -> {
                setLoading(false);
                showStatus(getString(R.string.online_error, error));
              });
        }
      };
