...
public final class PushMessagingService extends FirebaseMessagingService {
    ...
    @Override public void onNewToken(@NonNull String token){PushDeviceRegistrar.registerToken(this,token);}
    @Override public void onMessageReceived(@NonNull RemoteMessage message){
        Map<String,String> data=message.getData();String kind=value(data,"kind");
        long id=parseLong(value(data,"notification_id"),System.currentTimeMillis());
        Intent target=target(kind,data).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent pending=PendingIntent.getActivity(this,(int)(id%Integer.MAX_VALUE),target,
                PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);
        NotificationManager manager=getSystemService(NotificationManager.class);
        manager.createNotificationChannel(new NotificationChannel(CHANNEL,
                getString(R.string.notifications_title),NotificationManager.IMPORTANCE_DEFAULT));
        Notification notification=new Notification.Builder(this,CHANNEL)
                .setSmallIcon(R.mipmap.ic_launcher).setContentTitle(value(data,"title"))
                .setContentText(value(data,"body")).setStyle(new Notification.BigTextStyle()
                        .bigText(value(data,"body"))).setContentIntent(pending).setAutoCancel(true)
                .setCategory("room_call".equals(kind)?Notification.CATEGORY_CALL:Notification.CATEGORY_MESSAGE)
                .build();
    ...
    private static String value(Map<String,String> data,String key){
        String value=data.get(key);return value==null?"":value;
    }
