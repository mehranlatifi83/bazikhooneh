...
public final class LudoOnlineClient {
  ...
  public LudoOnlineClient(Context context, String base, String accountToken, Listener listener) {
  ...
  public void create(boolean thirdSixPenalty) {
    try {
      post("/api/v1/ludo/rooms/", new JSONObject().put("third_six_penalty", thirdSixPenalty));
              ...
              public void onResponse(Call call, Response response) {
                try (Response ignored = response) {
                  ...
                  if (json.has("reconnect_token")) {
                    ...
                    connect(json.optString("room_code"), json.optString("reconnect_token"));
                  } else {
  ...
  private void connect(String code, String token) {
    Request request =
        new Request.Builder()
            .url(base.replaceFirst("^http", "ws") + "/ws/v1/ludo/" + code + "/")
  ...
  private void send(String type, int piece) {
    try {
      JSONObject message =
          new JSONObject().put("type", type).put("action_id", UUID.randomUUID().toString());
