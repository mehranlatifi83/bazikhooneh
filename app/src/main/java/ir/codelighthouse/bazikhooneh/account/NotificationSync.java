...
public final class NotificationSync {
  ...
  private static void showNewest(Context context, JSONObject json) {
  ...
  private static void show(Context context, JSONObject item, long id) {
    ...
    Class<?> destination =
        "friend_request".equals(kind) ? FriendsActivity.class : NotificationsActivity.class;
    if ("game_invite".equals(kind) || "room_invite".equals(kind))
      destination = RoomListActivity.class;
    else if (data != null && data.has("room_code") && kind.startsWith("room_"))
      destination = RoomActivity.class;
