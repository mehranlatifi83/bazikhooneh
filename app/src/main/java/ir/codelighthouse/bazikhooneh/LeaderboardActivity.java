...
public final class LeaderboardActivity extends NavigableActivity {
    @Override protected void onCreate(Bundle state){super.onCreate(state);setContentView(R.layout.activity_leaderboard);
        ...
        client.leaderboard((data,error)->runOnUiThread(()->{findViewById(R.id.leaderboard_progress).setVisibility(View.GONE);
            if(error!=null){status.setText(error);status.announceForAccessibility(error);return;}JSONArray values=data.optJSONArray("results");
            if(values==null||values.length()==0){status.setText(R.string.empty_leaderboard);return;}status.setText("");for(int i=0;i<values.length();i++){JSONObject item=values.optJSONObject(i);TextView row=new TextView(this);
                row.setText(getString(R.string.leaderboard_item,i+1,item.optString("display_name"),item.optString("username"),item.optInt("score"),item.optInt("tic_tac_toe_wins"),item.optInt("ludo_wins")));
                row.setTextSize(17);row.setPadding(12,16,12,16);row.setFocusable(true);list.addView(row);}}));}
}
