...
import ir.codelighthouse.bazikhooneh.call.MediasoupCallEngine;
...
public final class CallKeepAliveService extends Service implements CommunityClient.Events {
    ...
    private void startMediaIfNeeded(){
        ...
        SessionStore store=new SessionStore(this);
        mediaEngine=new MediasoupCallEngine(this,BuildConfig.API_BASE_URL,store.token(),code,
                factory,audioTrack,new MediasoupCallEngine.Listener(){
            @Override public void onConnected(){main.post(()->{connected=true;
                statusText=getString(R.string.call_connected);publishAndAnnounce(statusText);});}
            @Override public void onDisconnected(){main.post(()->{if(!leaving)markReconnecting();});}
            @Override public void onRemoteAudio(String accountId){main.post(CallKeepAliveService.this::publish);}
        });
        mediaEngine.connect();
    }
    ...
    private void removePeer(String id,boolean shouldAnnounce){
        String name=names.remove(id);usernames.remove(id);micStates.remove(id);
    ...
    private void cleanup(){
        ...
        if(mediaEngine!=null){mediaEngine.close();mediaEngine=null;}
