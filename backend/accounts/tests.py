...
class AccountApiTests(APITestCase):
    ...
    def test_register_profile_update_and_logout(self):
        registered = self.client.post(
            "/api/v1/accounts/register/", self.registration, format="json"
        )
    ...
    def test_untrusted_forwarded_ip_is_not_logged(self):
        ...
        self.assertEqual(
            "203.0.113.10", str(SecurityEvent.objects.latest("created_at").ip_address)
        )
    ...
    def test_profile_stats_and_match_history(self):
        ...
        self.assertEqual(
            "three_piece_tic_tac_toe", profile.data["stats"]["by_game"][0]["game_key"]
        )
    ...
    def test_password_change_revokes_other_sessions(self):
        ...
        self.assertTrue(
            account.__class__.objects.get(pk=account.pk).check_password("new-password")
        )
    ...
    def test_email_verification_and_password_reset(self):
        ...
        self.assertTrue(
            Account.objects.get(pk=account.pk).check_password("reset-password")
        )
    ...
    def test_friend_request_presence_and_game_invite(self):
        ...
        self.assertEqual(
            201,
            self.client.post(
                "/api/v1/accounts/friends/", {"username": "second_user"}, format="json"
            ).status_code,
        )
        self.client.credentials(
        ...
        self.assertEqual(
            200,
            self.client.post(
                f"/api/v1/accounts/friends/requests/{request_id}/accept/"
            ).status_code,
        )
    ...
    def test_superuser_is_the_same_game_account(self):
        ...
        self.client.credentials()
        self.assertTrue(
            self.client.login(username="site_admin", password="admin-password")
        )
    ...
    def test_search_reject_remove_block_report_and_notifications(self):
        ...
        self.assertEqual(
            1,
            len(
                self.client.get("/api/v1/accounts/users/search/?q=second").data[
                    "results"
                ]
            ),
        )
        ...
        request_id = self.client.get("/api/v1/accounts/friends/").data["requests"][0][
            "request_id"
        ]
        self.assertEqual(
            204,
            self.client.delete(
                f"/api/v1/accounts/friends/requests/{request_id}/"
            ).status_code,
        )
        self.assertEqual(
            201,
            self.client.post(
                "/api/v1/accounts/blocks/", {"username": "first_user"}, format="json"
            ).status_code,
        )
        self.assertEqual(
            201,
            self.client.post(
                ...
                {"username": "first_user", "reason": "abuse"},
                format="json",
            ).status_code,
        )
