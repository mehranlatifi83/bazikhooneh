...
from .models import Match, Player, Room, LudoRoom, LudoSeat, LudoMatch
...
class CreateRoomView(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            player_payload(room, player, token), status=status.HTTP_201_CREATED
        )
...
class JoinRoomView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "room_not_found"}, status=status.HTTP_404_NOT_FOUND
            )
        if room.state != Room.State.WAITING or room.players.count() >= 2:
            return Response(
                {"error": "room_unavailable"}, status=status.HTTP_409_CONFLICT
            )

        if room.players.filter(account=request.user).exists():
            return Response(
                {"error": "already_in_room"}, status=status.HTTP_409_CONFLICT
            )
        ...
        ).update(accepted_at=room.updated_at)
...
class MatchHistoryView(APIView):
    ...
    def get(self, request):
        ...
        ludo_matches = (
            LudoMatch.objects.select_related("room", "winner")
        ...
        "seats": [
            {
                ...
                if s.account
                else f"Bot {s.color + 1}",
            }
            for s in room.seats.select_related("account").order_by("color")
        ],
    }
    if seat is not None:
        data.update(
            color=seat.color,
            reconnect_token=token,
            websocket_path=f"/ws/v1/ludo/{room.code}/",
        )
    return data
...
class CreateLudoRoomView(APIView):
    ...
    @transaction.atomic
    def post(self, request):
        room = LudoRoom.create_unique(request.user)
        room.game_state = {
            "third_six_penalty": bool(request.data.get("third_six_penalty", False))
        }
        room.save(update_fields=("game_state", "updated_at"))
        seat, token = LudoSeat.create_human(room, 0, request.user)
        return Response(ludo_payload(room, seat, token), status=201)
...
class JoinLudoRoomView(APIView):
    ...
    def post(self, request):
        ...
        room = LudoRoom.objects.select_for_update().filter(code=code).first()
        if not room:
            return Response({"error": "room_not_found"}, status=404)
        if room.state != "waiting" or room.seats.count() >= 4:
            return Response({"error": "room_unavailable"}, status=409)
        if room.seats.filter(account=request.user).exists():
            return Response({"error": "already_in_room"}, status=409)
        used = set(room.seats.values_list("color", flat=True))
        color = next(value for value in range(4) if value not in used)
        seat, token = LudoSeat.create_human(room, color, request.user)
        return Response(ludo_payload(room, seat, token))
...
class StartLudoRoomView(APIView):
    ...
    def post(self, request, code):
        room = (
            LudoRoom.objects.select_for_update()
            .filter(code=code.upper(), host=request.user, state="waiting")
            .first()
        )
        if not room:
            return Response({"error": "room_unavailable"}, status=409)
        ...
        for color in range(4):
            if color not in used:
                LudoSeat.objects.create(room=room, color=color, is_bot=True)
        bots = [False] * 4
        for seat in room.seats.all():
            bots[seat.color] = seat.is_bot
        penalty = bool(room.game_state.get("third_six_penalty", False))
        room.game_state = initial_state([True] * 4, bots, penalty)
        room.state = "active"
        room.version += 1
        room.save()
        match = LudoMatch.objects.create(room=room)
        match.participants.set(
            room.seats.filter(account__isnull=False).values_list(
        ...
        transaction.on_commit(
            lambda: async_to_sync(get_channel_layer().group_send)(
                f"ludo_{room.code}",
                {"type": "ludo.state", "payload": ludo_payload(room)},
            )
        )
