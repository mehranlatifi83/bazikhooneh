...
class CommunityRoomApiTests(APITestCase):
    ...
    def test_room_survives_switching_between_games(self):
        ...
        ludo = self.client.post(
            f"/api/v1/community/rooms/{code}/game/",
            {
                "game_key": "ludo",
                "third_six_penalty": True,
            },
            format="json",
        )
        ...
        self.assertEqual(
            "ended", room.game_sessions.order_by("created_at").first().state
        )
    ...
    def test_moderation_and_optional_call(self):
        ...
        code = self.client.post(
            "/api/v1/community/rooms/", {"title": "Room"}, format="json"
        ).data["code"]
        ...
        self.assertEqual(
            200,
            self.client.post(
                "/api/v1/community/rooms/join/", {"code": code}, format="json"
            ).status_code,
        )

        self.authenticate(owner_token)
        call = self.client.post(
            f"/api/v1/community/rooms/{code}/call/",
            {
                "mic_policy": "request",
                "max_participants": 4,
            },
        ...
        muted = self.client.post(
            f"/api/v1/community/rooms/{code}/moderate/",
            {
    ...
    def test_chat_message_has_reliable_rest_delivery_and_history(self):
        ...
        code = self.client.post(
            "/api/v1/community/rooms/", {"title": "Reliable chat"}, format="json"
        ).data["code"]
        sent = self.client.post(
            f"/api/v1/community/rooms/{code}/messages/",
    ...
    def test_member_can_invite_friend_to_existing_room(self):
        ...
        self.authenticate(owner_token)
        code = self.client.post(
            "/api/v1/community/rooms/", {"title": "Friends room"}, format="json"
        ).data["code"]
        ...
        Friendship.objects.create(
            requester=owner, recipient=friend, status=Friendship.STATUS_ACCEPTED
        )
        self.authenticate(owner_token)
        response = self.client.post(
            f"/api/v1/community/rooms/{code}/invite/",
            {"username": "invited_friend"},
            format="json",
        )
        self.assertEqual(201, response.status_code)
        self.assertTrue(
            AccountNotification.objects.filter(
                account=friend, kind="room_invite", data__room_code=code
            ).exists()
        )
    ...
    def test_banned_member_cannot_rejoin(self):
        ...
        self.authenticate(owner_token)
        code = self.client.post("/api/v1/community/rooms/", {}, format="json").data[
            "code"
        ]
        ...
        self.client.post(
            f"/api/v1/community/rooms/{code}/moderate/",
            {"username": "banned_member", "action": "ban"},
            format="json",
        )
        self.authenticate(member_token)
        self.assertEqual(
    ...
    def test_tic_matchmaking_starts_game_for_both_players(self):
        ...
        self.assertEqual(
            matched.data["player"]["game"]["room_code"],
    ...
    def test_member_joins_game_through_shared_room(self):
        ...
        joined = self.client.post(
            f"/api/v1/community/rooms/{code}/game/join/", {}, format="json"
        )
    ...
    def test_ludo_matchmaking_creates_shared_game_session(self):
        ...
        self.assertEqual(
            202,
            self.client.post(
                "/api/v1/matchmaking/", {"game_key": "ludo"}, format="json"
            ).status_code,
        )
...
class CommunityRoomWebSocketTests(APITestCase):
    def setUp(self):
        ...
        self.code = self.client.post(
            "/api/v1/community/rooms/", {"title": "Live room"}, format="json"
        ).data["code"]
